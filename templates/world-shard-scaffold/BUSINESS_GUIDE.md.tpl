...
A safe first change for the world-shard scaffold is to add one local migration pre-check or shard routing rule.

Suggested path:

1. Extend `__PROTOCOL_FILE__`, for example add a `requiredLevel` field to transfer or a `queryShardLoad(worldId, traceId)` request.
2. Reserve a protocol id in `src/main/protocol/protoId.txt` if you add a new request.
3. Run `mvn -q generate-sources`.
4. Update the handwritten BO logic in `__APP_CLASS__` so entity and shard state still mutate through the existing local Actor lane.
5. Add one assertion to `__TEST_CLASS__` that verifies the entity shard, migration count or shard summary.

Keep the first change as a local routing rule. Do not add cross-process handoff, distributed locks, cross-server broadcast or consistency protocols here.
