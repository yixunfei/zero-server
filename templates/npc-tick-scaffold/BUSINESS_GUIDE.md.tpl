...
A safe first change for the NPC tick scaffold is to add one simple behavior transition.

Suggested path:

1. Extend `__PROTOCOL_FILE__`, for example add a `setBehavior(uid, npcId, behavior, traceId)` variant or a small behavior parameter.
2. Reserve a protocol id in `src/main/protocol/protoId.txt` if you add a new request.
3. Run `mvn -q generate-sources`.
4. Update the handwritten BO logic in `__APP_CLASS__` so tick updates remain inside the existing zone Actor lane.
5. Add one assertion to `__TEST_CLASS__` that verifies the NPC summary includes the new behavior or tick outcome.

Keep the first change as deterministic local behavior. Do not add behavior trees, pathfinding, combat AI, distributed tick budgets or migration here.
