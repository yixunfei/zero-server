...
当前状态：`implemented-local-minimum-slice / productionReady=false`。对应模块：`zero-frame-sync`，入口：`FrameMatchRuntime`。使用方式见[可运行示例](../../examples/frame-sync/README.md)，整体状态见[能力矩阵](../capability-matrix.zh-CN.md)。

本页先列当前本地限制和验证口径。末尾保留历史设计输入，其中候选类型、状态和跨服行为可能尚未实现，不应直接作为现行 API 使用。生产扩展仍需评审公共 API、Actor 所有权和数据边界；`requiresConfirmation=true`。

<!-- contract-id=frame-sync-minimum-contract -->
...
frame-sync scaffold
  ...
  -> docs/reference/frame-sync-minimum-contract.zh-CN.md
...
targetFrame <= 已提交帧号视为迟到；REJECT 每次明确拒绝，MARK 丢弃且在近期窗口记录，BUFFER 转入下一帧。同一玩家目标帧的替换不增加容量；只在接受成功后登记幂等键。近期去重窗口最多 maxBufferedInputs 个键，窗口之外的历史输入依照时序策略处理，不承诺全局永久去重。多个 FrameMatchRuntime 可共用一个 ActorScheduler，通过 FrameDispatchRegistry 的无状态命令路由器进入各自 lane；关闭某个对局不影响其他对局。
