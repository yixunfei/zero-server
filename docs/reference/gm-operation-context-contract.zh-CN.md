# zeroServer GM 操作上下文与审计契约

本文保留授权最小切片的设计和接口边界。当前完整接入说明见[GM 指南](../gm-admin.zh-CN.md)；后续增加的 REST 适配、标准入口、审计查询和幂等能力见[标准入口契约](gm-standard-entry-persistence-contract.zh-CN.md)与[迁移记录](../migrations/README.md)。下面的“该切片不实现”仅描述最初授权切片，不代表后续模块均未实现。
...
审计日志候选字段应复用 `docs/reference/observability-minimum-field-contract.zh-CN.md` 的基础字段，并补充 GM 维度字段：
