...
关联切片：`aoi-state-sync-minimum-contract`

本文是 `aoi-state-sync` 从 local/prototype 模板走向正式组件前的最小契约输入。它只描述候选语义、风险边界和 focused tests，不创建 `zero-aoi` / `zero-state-sync` 模块，不冻结公共 API，不改变 Actor 线程模型、协议、RPC、存储、缓存、日志字段或 ErrorCode 结构。

场景移动性能前置口径见 `docs/scene-move-loop-performance-evidence.zh-CN.md` 与 `scripts/ZeroSceneMoveLoopBenchmarkReadiness.java`。该口径只把 scene-sync 线性可见性查询作为 prototype 分层，不证明正式 AOI、delta、广播或容量达标。
...
scene-sync scaffold
  ...
  -> docs/aoi-state-sync-minimum-contract.zh-CN.md
