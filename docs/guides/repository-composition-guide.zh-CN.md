...
日志与 Failsafe 摘要仍写入唯一运行目录；`resilience.log` 包含每 30 秒进度、成功数、冲突数、含重试的平均/最大延迟。五分钟验证不能证明小时级资源稳定性，也不覆盖请求执行中断网和写入结果对账。最新实际结果与下一步见[推进方案](../reports/demand-composition-2026-09-12.zh-CN.md)。
