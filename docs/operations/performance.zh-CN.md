...
以下七份材料保留不同路径的负载矩阵、测量字段和失效场景，属于测量方案，不是已经取得的性能结果：

| 测量对象 | 负载与采集口径 |
| --- | --- |
| 协议 | [Codec payload 矩阵](evidence/protocol-codec-performance-evidence.zh-CN.md) |
| Actor | [调度与队列](evidence/actor-scheduler-performance-evidence.zh-CN.md) |
| 网络 | [帧处理与本机回环](evidence/net-frame-performance-evidence.zh-CN.md) |
| RPC | [Pending 与超时](evidence/rpc-pending-performance-evidence.zh-CN.md) |
| 数据 | [Repository、CAS 与刷写](evidence/repository-save-performance-evidence.zh-CN.md) |
| 缓存 | [分层命中与加载背压](evidence/cache-get-or-load-performance-evidence.zh-CN.md) |
| 场景 | [移动、快照与 Actor 投递](evidence/scene-move-loop-performance-evidence.zh-CN.md) |
