...
2026-09-23 本地实现：注册/注销在短写锁内发布一个不可变快照，单次 publish 的拦截器和处理器来自同一版本。回调在写锁外，标准已完成 Future 迭代处理，未完成或通用 CompletionStage 交接后继续原快照。每次完成信号独立；失败继续派发并汇总死信/异常，无需业务选择新的同步接口。
