...
| `ZeroBuildSmokeVerifier` | 已有 | 根 reactor 与示例可显式执行 `validate` | 单元测试、质量门禁、性能或生产验收通过 |
| [发布检查单](release-checklist.zh-CN.md) | 本切片新增 | 发布前检查维度被统一列出 | 检查项已经由维护者逐项签字 |
| [迁移说明模板](migration-guide-template.zh-CN.md) | 本切片新增 | 破坏性变更有最低记录结构 | 某个具体版本已经完成迁移说明 |
...
```text
zero-release-hardening-readiness=ok|paths=18/18|markers=31/31|levels=7|requiresConfirmation=true|warnings=0
```
...
验证器不会启动 Maven 或子进程，不会访问网络，也不会修改仓库。
...
3. 若有破坏性变更，从 [迁移说明模板](migration-guide-template.zh-CN.md) 创建具体迁移说明，不直接改写模板为某个版本的结果。
