...
- PNFT-01～PNFT-10 已落为 JUnit focused tests；映射见 `docs/production-network-focused-tests.zh-CN.md`。
...
focused tests 及真实 JUnit 落点见 `docs/production-network-focused-tests.zh-CN.md`，可用 `scripts/ZeroProductionNetworkFocusedTestPlan.java` 只读校验当前 `implementationAllowed=true|confirmed=true|productionReady=false` 证据。该脚本自身不创建测试类或修改运行时。
