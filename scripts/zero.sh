...
test_cmd() {
  ...
  if [[ -n "$project_dir" ]]; then
    ...
    if ((${#args[@]})); then
      ...
      (cd "$ROOT_DIR" && "$(mvn_cmd)" -B -ntp -f "$project_pom" test)
    fi
  else
    if ((${#args[@]})); then
      (cd "$ROOT_DIR" && "$(mvn_cmd)" -B -ntp test "${args[@]}")
    else
      (cd "$ROOT_DIR" && "$(mvn_cmd)" -B -ntp test)
    fi
  fi
}
...
stop_cmd() {
  ...
  if kill -0 "$pid" 2>/dev/null; then
    ...
    if [[ -z "$command_line" || ( "$command_line" != *mvn* && "$command_line" != *mvnw*
        && ( -z "$project_dir" || ( "$command_line" != *"$project_dir"* && "$process_cwd" != "$project_dir" ) ) ) ]]; then
